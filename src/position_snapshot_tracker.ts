...
import { VirtualPositionManager, type VirtualPosition } from './virtual_position_mgr';
...
export class PositionSnapshotTracker {
    ...
    private createPositionSnapshot(position: VirtualPosition, timestamp: number): PositionSnapshot {
        ...
        return {
            ...
            performance: {
                unrealizedPnL,
                unrealizedPnLPct: totalValueUSD > 0 ? (unrealizedPnL / totalValueUSD) * 100 : 0,
                realizedPnL,
                feeYield: this.calculateFeeYield(position),
                totalReturn: unrealizedPnL + realizedPnL,
                totalReturnPct: totalValueUSD > 0 ? ((unrealizedPnL + realizedPnL) / totalValueUSD) * 100 : 0,
                timeInRange,
                timeOutOfRange,
            ...
            utilization: {
                ...
                impermanentLoss,
                impermanentLossPct: holdingValue > 0 ? (impermanentLoss / holdingValue) * 100 : 0,
                holdingValue,
    ...
    private createSummarySnapshot(positionSnapshots: PositionSnapshot[], timestamp: number): PositionSummarySnapshot {
        ...
        const activePositions = positionSnapshots.filter(p => p.liquidity.isActive).length;
        const inRangePositions = positionSnapshots.filter(p => p.liquidity.inRange).length;
        const outOfRangePositions = totalPositions - inRangePositions;

        const belowRange = positionSnapshots.filter(p => p.priceInfo.pricePosition === 'below').length;
        const aboveRange = positionSnapshots.filter(p => p.priceInfo.pricePosition === 'above').length;

        // Calculate aggregated metrics
        const totalValueUSD = positionSnapshots.reduce((sum, p) => sum + p.tokens.totalValueUSD, 0);
        const totalFeesUSD = positionSnapshots.reduce((sum, p) => sum + p.fees.totalFeesUSD, 0);
        const totalUnrealizedPnL = positionSnapshots.reduce((sum, p) => sum + p.performance.unrealizedPnL, 0);
        const totalRealizedPnL = positionSnapshots.reduce((sum, p) => sum + p.performance.realizedPnL, 0);
        const totalReturn = totalUnrealizedPnL + totalRealizedPnL;
        const totalImpermanentLoss = positionSnapshots.reduce((sum, p) => sum + p.utilization.impermanentLoss, 0);
        const totalHoldingValue = positionSnapshots.reduce((sum, p) => sum + p.utilization.holdingValue, 0);

        // Calculate portfolio volatility
        const portfolioVolatility = this.calculatePortfolioVolatility(positionSnapshots);
        const maxDrawdown = this.calculateMaxDrawdown();
        const valueAtRisk = this.calculateValueAtRisk(positionSnapshots);
        const expectedShortfall = this.calculateExpectedShortfall(positionSnapshots);

        // Swap analysis aggregation
        const swapAnalyses = positionSnapshots.filter(p => p.swapAnalysis).map(p => p.swapAnalysis!);
        const totalSwapOpportunities = swapAnalyses.length;
        const totalSwapValue = swapAnalyses.reduce((sum, s) => sum + s.totalSwapValue, 0);
        const avgSwapEfficiency = swapAnalyses.length > 0 ?
            swapAnalyses.reduce((sum, s) => sum + s.swapEfficiency, 0) / swapAnalyses.length : 0;
        const roundTripsAvoided = swapAnalyses.filter(s => s.avoidedRoundTrips).length;
        ...
        return {
            ...
            inRangePositions,
            outOfRangePositions,
            totalLiquidity: positionSnapshots.reduce((sum, p) => sum + BigInt(p.liquidity.amount), 0n).toString(),
            totalValueUSD,
            totalFeesUSD,
            averageTickWidth: totalPositions > 0 ?
                positionSnapshots.reduce((sum, p) => sum + p.tickRange.tickWidth, 0) / totalPositions : 0,
            positionDistribution: {
                below: belowRange,
            ...
            performanceMetrics: {
                avgUnrealizedPnL: totalPositions > 0 ? totalUnrealizedPnL / totalPositions : 0,
                avgUnrealizedPnLPct: totalPositions > 0 ?
                    positionSnapshots.reduce((sum, p) => sum + p.performance.unrealizedPnLPct, 0) / totalPositions : 0,
                avgRealizedPnL: totalPositions > 0 ? totalRealizedPnL / totalPositions : 0,
                avgFeeYield: totalPositions > 0 ?
                    positionSnapshots.reduce((sum, p) => sum + p.performance.feeYield, 0) / totalPositions : 0,
                avgFeeYieldAPR: totalPositions > 0 ?
                    positionSnapshots.reduce((sum, p) => sum + p.fees.feeYieldAPR, 0) / totalPositions : 0,
                avgTimeInRange: totalPositions > 0 ?
                    positionSnapshots.reduce((sum, p) => sum + p.performance.timeInRange, 0) / totalPositions : 0,
                avgTimeInRangePct: totalPositions > 0 ?
                    positionSnapshots.reduce((sum, p) => sum + p.performance.timeInRangePct, 0) / totalPositions : 0,
                totalImpermanentLoss,
                totalImpermanentLossPct: totalHoldingValue > 0 ? (totalImpermanentLoss / totalHoldingValue) * 100 : 0,
                avgROI: totalPositions > 0 ?
                    positionSnapshots.reduce((sum, p) => sum + p.performance.roi, 0) / totalPositions : 0,
                avgSharpeRatio: totalPositions > 0 ?
                    positionSnapshots.reduce((sum, p) => sum + p.performance.sharpeRatio, 0) / totalPositions : 0,
                totalReturn,
                totalReturnPct: totalValueUSD > 0 ? (totalReturn / totalValueUSD) * 100 : 0
            },
            riskMetrics: {
                portfolioVolatility,
                maxDrawdown,
    ...
    private generatePositionId(position: VirtualPosition): string {
        return `${position.tickLower}_${position.tickUpper}_${position.liquidity.toString().slice(-8)}`;
    }
    ...
    private calculateAccruedFees(position: VirtualPosition, tokenIndex: 0 | 1): bigint {
        // Enhanced fee calculation based on fee growth
        const poolFeeGrowthGlobal = tokenIndex === 0 ?
            (this.pool as any).feeGrowthGlobal0X64 || 0n :
            (this.pool as any).feeGrowthGlobal1X64 || 0n;

        const positionFeeGrowthInside = tokenIndex === 0 ?
    ...
    private calculateTotalFeesUSD(position: VirtualPosition): number {
        const fees0USD = this.calculateTokenValue(this.calculateAccruedFees(position, 0), 'A');
        const fees1USD = this.calculateTokenValue(this.calculateAccruedFees(position, 1), 'B');
        return fees0USD + fees1USD;
    }

    private calculateUnrealizedPnL(position: VirtualPosition): number {
        const currentValue = this.calculateTokenValue(position.tokensOwed0, 'A') +
    ...
    private getPositionInitialValue(position: VirtualPosition): number {
        ...
        const midPrice = (this.tickToPrice(position.tickLower) + this.tickToPrice(position.tickUpper)) / 2;
    ...
    private getTimeInRange(position: VirtualPosition, currentTime: number): number {
        const positionId = this.generatePositionId(position);
        return this.positionInRangeTimes.get(positionId) || 0;
    }

    private getTimeOutOfRange(position: VirtualPosition, currentTime: number): number {
        const positionId = this.generatePositionId(position);
    ...
    private calculateCapitalEfficiency(position: VirtualPosition): number {
        const totalFees = this.calculateTotalFeesUSD(position);
        const totalValue = this.calculateTokenValue(position.tokensOwed0, 'A') +
            this.calculateTokenValue(position.tokensOwed1, 'B');
    ...
    private analyzeSwapOpportunities(position: VirtualPosition): {
        ...
        const totalSwapValue = swaps.reduce((sum, swap) => sum + swap.swapValueUSD, 0);
        const avgEfficiency = swaps.length > 0 ?
            swaps.reduce((sum, swap) => sum + swap.efficiency, 0) / swaps.length : 0;
    ...
    private calculateImpermanentLoss(position: VirtualPosition): number {
        ...
        const positionValue = this.calculateTokenValue(position.tokensOwed0, 'A') +
            this.calculateTokenValue(position.tokensOwed1, 'B');
    ...
    private calculateHoldingValue(position: VirtualPosition): number {
        ...
        const holdingValue = (initialValue0 * currentPrice / entryPrice) + initialValue1;
    ...
    private updateTimeTracking(positionId: string, snapshot: PositionSnapshot, timestamp: number): void {
        if (snapshot.liquidity.inRange) {
            const currentInRangeTime = this.positionInRangeTimes.get(positionId) || 0;
            this.positionInRangeTimes.set(positionId, currentInRangeTime + this.snapshotInterval);
        }
    }
    ...
    public saveSnapshots(filename?: string): void {
        ...
        const baseFilename = filename ? filename.replace('.json', '') : `position_snapshots_${timestamp}`;
    ...
    private generateOverallSummary() {
        ...
        return {
            ...
            positionStats: {
                maxPositions: Math.max(...this.summarySnapshots.map(s => s.totalPositions)),
                minPositions: Math.min(...this.summarySnapshots.map(s => s.totalPositions)),
                avgPositions: this.summarySnapshots.reduce((sum, s) => sum + s.totalPositions, 0) / this.summarySnapshots.length,
                avgInRangePositions: this.summarySnapshots.reduce((sum, s) => sum + s.inRangePositions, 0) / this.summarySnapshots.length,
                avgInRangePercentage: this.summarySnapshots.reduce((sum, s) =>
                    sum + (s.totalPositions > 0 ? (s.inRangePositions / s.totalPositions) * 100 : 0), 0) / this.summarySnapshots.length
            },
            valueStats: {
                maxValue: Math.max(...this.summarySnapshots.map(s => s.totalValueUSD)),
                minValue: Math.min(...this.summarySnapshots.map(s => s.totalValueUSD)),
                avgValue: this.summarySnapshots.reduce((sum, s) => sum + s.totalValueUSD, 0) / this.summarySnapshots.length,
                totalFees: last.totalFeesUSD
            },
            performanceStats: {
                avgUnrealizedPnL: this.summarySnapshots.reduce((sum, s) => sum + s.performanceMetrics.avgUnrealizedPnL, 0) / this.summarySnapshots.length,
                avgFeeYield: this.summarySnapshots.reduce((sum, s) => sum + s.performanceMetrics.avgFeeYield, 0) / this.summarySnapshots.length,
                avgTimeInRange: this.summarySnapshots.reduce((sum, s) => sum + s.performanceMetrics.avgTimeInRange, 0) / this.summarySnapshots.length,
