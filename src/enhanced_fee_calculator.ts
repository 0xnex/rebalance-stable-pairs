...
export class EnhancedFeeCalculator {
  ...
  calculatePreciseFees(
    ...
    const compoundedFees = this.calculateCompoundedFees(
      position,
      startTime,
      endTime
    );
    ...
    const totalFeesToken0 =
      baseFees.fee0 + tickCorrections.fee0 + compoundedFees.fee0;
    const totalFeesToken1 =
      baseFees.fee1 + tickCorrections.fee1 + compoundedFees.fee1;
  ...
  private calculateBaseFees(position: any): { fee0: bigint; fee1: bigint } {
    ...
    const snapshots = this.getFeeGrowthSnapshots(
      position.tickLower,
      position.tickUpper
    );
    ...
    for (let i = 1; i < snapshots.length; i++) {
      ...
      const feeGrowthDelta0 =
        currSnapshot.feeGrowthGlobal0 - prevSnapshot.feeGrowthGlobal0;
      const feeGrowthDelta1 =
        currSnapshot.feeGrowthGlobal1 - prevSnapshot.feeGrowthGlobal1;

      // Weight by time and position activity
      const weight = this.calculatePositionWeight(
        position,
        prevSnapshot.timestamp,
        currSnapshot.timestamp
      );

      totalFee0 +=
        (position.liquidity * feeGrowthDelta0 * BigInt(weight)) /
        this.Q64 /
        1000n;
      totalFee1 +=
        (position.liquidity * feeGrowthDelta1 * BigInt(weight)) /
        this.Q64 /
        1000n;
    }
   ...
   */
  private calculateTickLevelCorrections(position: any): {
    fee0: bigint;
    fee1: bigint;
  } {
  ...
  private calculateTimeWeightedFees(
    ...
    for (let i = 1; i < relevantSnapshots.length; i++) {
      ...
      const priceStability =
        1 / (1 + Math.abs(curr.price - prev.price) / prev.price);

      const weight = timeDelta * volumeWeight * priceStability;

      const feeContrib0 =
        ((curr.feeGrowthGlobal0 - prev.feeGrowthGlobal0) * position.liquidity) /
        this.Q64;
      const feeContrib1 =
        ((curr.feeGrowthGlobal1 - prev.feeGrowthGlobal1) * position.liquidity) /
        this.Q64;
  ...
  private calculateCompoundedFees(
    ...
    const reinvestmentEvents = this.getReinvestmentEvents(
      position.id,
      startTime,
      endTime
    );
    ...
    for (const event of reinvestmentEvents) {
      ...
      compoundedFee0 +=
        (event.reinvestedFee0 * BigInt(Math.floor(compoundRate * 1000))) /
        1000n;
      compoundedFee1 +=
        (event.reinvestedFee1 * BigInt(Math.floor(compoundRate * 1000))) /
        1000n;
    }
  ...
  private validateFeesAgainstChain(
    ...
    const accuracy0 = this.calculateAccuracy(
      calculatedFees.fee0,
      onChainFees.fee0
    );
    const accuracy1 = this.calculateAccuracy(
      calculatedFees.fee1,
      onChainFees.fee1
    );
  ...
  private getFeeGrowthSnapshots(tickLower: number, tickUpper: number) {
    return this.feeGrowthSnapshots.filter(
      (s) =>
  ...
  private calculateTickCrossingCorrection(
    tick: number,
    position: any
  ): { fee0: bigint; fee1: bigint } {
  ...
  private calculateAccuracy(calculated: bigint, actual: bigint): number {
    ...
    const diff =
      calculated > actual ? calculated - actual : actual - calculated;
    const accuracy = 1 - Number(diff) / Number(actual);
    return Math.max(0, Math.min(1, accuracy));
  }

  private calculateAverageFeeRate(
    position: any,
    startTime: number,
    endTime: number
  ): number {
  ...
  updateFeeGrowthSnapshot(timestamp: number, volume: bigint, price: number) {
    ...
    this.feeGrowthSnapshots = this.feeGrowthSnapshots.filter(
