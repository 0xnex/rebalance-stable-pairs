...
export function strategyFactory(pool: Pool): BacktestStrategy {
  ...
  // Create fee collection configuration if enabled
  ...
  const canRebalance = (
    ...
    if (dailyRebalanceCount >= env.maxDailyRebalances) {
      return {
        allowed: false,
        reason: `Daily limit reached (${env.maxDailyRebalances} rebalances)`,
      };
    }
  ...
  const getRebalancingCase = (
    ...
    if (currentPrice >= pos1Lower && currentPrice <= pos1Upper) {
      return {
        case: 3,
        description: "Price within Position 1 range - No action",
      };
    } else if (currentPrice > pos1Upper && currentPrice <= pos2Upper) {
      return {
        case: 1,
        description: "Price above Position 1 - Rebalance Position 1",
      };
    } else if (currentPrice < pos1Lower && currentPrice >= pos3Lower) {
      return {
        case: 1,
        description: "Price below Position 1 - Rebalance Position 1",
      };
    } else if (currentPrice > pos2Upper || currentPrice < pos3Lower) {
      return {
        case: 2,
        description: "Price outside all positions - Full rebalance",
      };
    } else {
      return {
        case: 1,
        description: "Price in Position 2/3 range - Rebalance Position 1",
      };
    }
  };
  ...
  const log = (
    ...
    ctx.logger?.log?.(
      `[three-band-option3] ${new Date(
        ...
        positions.length
      } inRange=${inRangeCount} | fees=(A:${totalFeesA} B:${totalFeesB}) | Case=${
        rebalancingCase.case
      } (${rebalancingCase.description}) | Rebalance: ${
        rebalanceCheck.allowed ? "ALLOWED" : "BLOCKED"
      } (${rebalanceCheck.reason})${feeCollectionInfo}`
    );
    ...
    // Get fee collection data for CSV
    ...
    const csvParts: string[] = [
      ...
      feeCollectionEvents.toString(),
      reinvestmentEvents.toString(),
      feeCollectionEfficiency.toFixed(4),
    ];
  ...
  const runOnce = (ctx: StrategyContext) => {
    ...
    let outcome;
    if (
      !rebalanceCheck.allowed &&
      (rebalancingCase.case === 1 || rebalancingCase.case === 2)
    ) {
