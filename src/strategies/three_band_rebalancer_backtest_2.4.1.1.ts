...
} from "./three_band_rebalancer_strategy_option_2.4.1.1";
...
type EnvConfig = {
  ...
  // V1.2 Option A specific configs
  ...
  baseRangeMultiplier: number;
  mediumRangeMultiplier: number;
  wideRangeMultiplier: number;
  trendScore: number;
  safetyScore: number;
...
function readEnvConfig(): EnvConfig {
  ...
  return {
    ...
    // V1.2 Option A specific configurations
    maxDailyRebalances: toNumber(process.env.THREEBAND_MAX_DAILY_REBALANCES, 1), // V1.2: 1 per day
...
export function strategyFactory(
  ...
  const csvFilePath = `three_band_v1.2_optionA_backtest_${token_a}_${token_b}_${initial_amount_a}_${initial_amount_b}_${Date.now()}.csv`;
  ...
  const config: Partial<ThreeBandRebalancerConfigOptionThree> = {
    ...
    // Range multipliers (contiguous bands)
    baseRangeMultiplier: env.baseRangeMultiplier,
    mediumRangeMultiplier: env.mediumRangeMultiplier,
    wideRangeMultiplier: env.wideRangeMultiplier,
    // Market indicators
    trendScore: env.trendScore,
    safetyScore: env.safetyScore,
  };
    ...
    if (action === "create") {
      for (let i = 0; i < positions.length; i++) {
        ...
        if (pos) {
          ...
          ctx.logger?.log?.(
            `[three-band-v1.2-optionA]   Position ${i + 1} (${allocation}%): [${
              pos.tickLower
    ...
    if (action === "wait" || action === "rebalance") {
      ...
      if (segments.length > 0) {
        ...
        if (firstSeg && lastSeg) {
          ...
          ctx.logger?.log?.(
            `[three-band-v1.2-optionA]   → Range: [${firstSeg.tickLower}, ${lastSeg.tickUpper}] | Status: ${rangeStatus} | Distance: lower=${lowerDist}ticks upper=${upperDist}ticks`
          );
          ...
          if (message.includes("Rebalance") || message.includes("cooldown")) {
            ...
            ctx.logger?.log?.(
              `[three-band-v1.2-optionA]   → V1.2 Option A Constraints: Daily=${dailyRemaining}/${env.maxDailyRebalances} | Cooldown=${cooldownStatus}`
            );
          }
        }
      }
  ...
  return {
    id: "three-band-rebalancer-v1.2-optionA",
    async onInit(ctx) {
      strategy.setCurrentTime(ctx.timestamp);
    ...
    async onFinish(ctx) {
      ...
      ctx.logger?.log?.(
        `[three-band-v1.2-optionA] BEFORE closing: positions=${beforeTotals.positions}`
      );
      ctx.logger?.log?.(
      ...
      ctx.logger?.log?.(
        `[three-band-v1.2-optionA] initial investment: initialA=${beforeTotals.initialAmountA} initialB=${beforeTotals.initialAmountB}`
      );
      ...
      const totals = manager.getTotals();
      ctx.logger?.log?.(
        `[three-band-v1.2-optionA] FINISH TOTALS (after closing all positions):`
      );
      ctx.logger?.log?.(
      ...
      ctx.logger?.log?.(
        `[three-band-v1.2-optionA] V1.2 Option A Summary: Total rebalances today=${dailyRebalanceCount}/${env.maxDailyRebalances}`
      );
    },
  };
}
